...
let tz = '';
let cash = $.getval('cash') || 1; //0为不自动提现,1为自动提现1元,5为自动提现1元
...
async function txstock(){
  ...
  console.log(`\n✅ 执行【App】新添加日常任务\n`)
  //修复 新任务
   await task10();  //查看今日大盘行情	  
   await task11();  //浏览行情港股
   await task12();  //阅读资讯
   await task13();  //查看热搜并添加自选   
   await task14();  //添加精选到自选
  ...
  console.log(`\n✅ 执行【微信小程序】新添加日常任务\n`)  
  //修复 新任务
    await wxtask12();  //WX阅读资讯
    await wxtask13();  //WX添加自选股
    await wxtask14();  //WX浏览社区
    await wxtask15();  //WX浏览选股题材热点
    await wxtask16();  //WX浏览股票扫雷
    //await wxtask17();  //WX分享个股行情	
    //await wxtask18();  //WX分享一篇资讯	
    await wxtask19();  //WX浏览行情港股	
    await wxtask20();  //WX查看今日大盘行情		
  console.log(`\n✅ 执行【自动提现】任务\n`)
...
async function update() {
  return new Promise((resolve) => {
    let updateurl = {
      url: `https://inf-prdapi.wesure.cn/prdapp/api/config-service/getConfig`,
      body: {"namespace": "wefit_config","key": "UPDATE_CONFIG","appID": "wefit",}
      headers: {
		"Accept": "*/*",
        ...
        "Host": "inf-prdapi.wesure.cn",
        "Referer": "https://servicewechat.com/wx06a561655ab8f5b2/660/page-frame.html",
        "User-Agent": "Mozilla/5.0 (iPad; CPU OS 13_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.7(0x18000731) NetType/WIFI Language/zh_CN",
        "Ws-Channel": "",
        "Ws-DeviceId": "83147162540594665071844",
        "Ws-Login-Appid": "wx06a561655ab8f5b2",
        "Ws-Login-Token": "www.wesure.cn",
        "Ws-Login-Type": "1",
        "Ws-Loginid": "",
        "Ws-Miniapp-Env": "prd",
        "Ws-Miniapp-Version": "3.32.1",
        "Ws-Platform": "wc-app",
        "Ws-Request-Id": "77c7327f",
        "Ws-Route": "",
        "Ws-User-Tag": "",
        "Ws-outer-id": "",
        "Ws-scene": "1089",
        "Ws-sessionId": "25432162588732187887443",
        "Ws-wtagid": "",
        "ws-proto-version": "2"
		},
    };
    $.post(updateurl, async (err, resp, data) => {
      try {
        if (err) {
          console.log("腾讯微保: API查询请求失败 ‼️‼️");
          ...
          if (safeGet(data)) {
            ...
            if(data.search == UPDATE_CONFIG){
              $.log(`【更新配置成功】`);
              tz += `【更新配置成功】\n`
              //await $.wait(5000); //等待5秒
            }else{
              console.log(`更新失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【更新配置】:${data.retmsg}\n`
            }
          }
        }
...
function user() {
  return new Promise((resolve) => {
    let url = {
      userurl: `https://api.wesure.cn/api/pub/V1/user`,
      body: {
        "requestId": "c176d78a",
        "appId": "wx06a561655ab8f5b2",
        "cmd": "UserWxAppLogin",
        "sessionId": "25432162588732187887443",
        "userId": "",
        "token": "www.wesure.cn",
        "version": "PRD_3.32.1_",
        "wtagId": "",
        "body": {
        "code": "01336z1w3FGrIW2c3u1w3fb8SC336z1V",
        "appId": "wx06a561655ab8f5b2",
        "includeFields": ["UserInfo","WxInfo","PhoneInfo"],
        "whiteList": [{"productCode": "","source": "","extraInfo": ""}]
            }
        },
      headers: {
		"Accept": "*/*",
        ...
        "Ws-Miniapp-Version": "3.32.1",
        "Ws-Platform": "wc-app",
        "Ws-Request-Id": "c176d78a",
        "Ws-Route": "pages/insure/main/launch/index",
        "Ws-User-Tag": "",
        "Ws-outer-id": "",
        "Ws-scene": "1089",
        "Ws-sessionId": "25432162588732187887443",
    ...
    $.post(userurl, async (err, resp, data) => {
      try {
        if (err) {
          console.log("腾讯微保: API查询请求失败 ‼️‼️");
          console.log(JSON.stringify(err));
          ...
          if (safeGet(data)) {
            ...
            $.log(`本次验证User🎫：${data.sessionID}`);
            $.log(`本次验证User🎫：${data.body.code}`);
...
async function task10() {
  console.log(`开始验证【查看今日大盘行情】任务状态`)
  await statuid1_new()
  if (statuid1_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【查看今日大盘行情】任务`)
    await taskid1_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【查看今日大盘行情】:已执行\n`
  }
...
async function task11() {
  console.log(`开始验证【浏览行情港股】任务状态`)
  await statuid2_new()
  if (statuid2_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【浏览行情港股】任务`)
    await taskid2_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【浏览行情港股】:已执行\n`
  }
...
async function task12() {
  console.log(`开始验证【阅读资讯】任务状态`)
  await statuid3_new()
  if (statuid3_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【阅读资讯】任务`)
    await taskid3_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【阅读资讯】:已执行\n`
  }
...
async function task13() {
  console.log(`开始验证【查看热搜并添加自选】任务状态`)
  await statuid4_new()
  if (statuid4_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【查看热搜并添加自选】任务`)
    await taskid4_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【查看热搜并添加自选】:已执行\n`
  }
...
//WX添加自选股
async function wxtask13() {
  console.log(`开始验证【WX添加自选股】任务状态`)
  await wxstatuid4_new()
  if (wxstatuid3_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX添加自选股】任务`)
    await wxtaskid4_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX添加自选股】:已执行\n`
  }
...
async function task14() {
  console.log(`开始验证【添加精选到自选】任务状态`)
  await statuid5_new()
  if (statuid5_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【添加精选到自选】任务`)
    await taskid5_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【添加精选到自选】:已执行\n`
  }
...
//WX浏览社区
async function wxtask14() {
  console.log(`开始验证【WX浏览社区】任务状态`)
  await wxstatuid7_new()
  if (wxstatuid3_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX浏览社区】任务`)
    await wxtaskid7_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX浏览社区】:已执行\n`
  }
...
async function task15() {
  console.log(`开始验证【添加龙虎榜到自选】任务状态`)
  await statuid6_new()
  if (statuid6_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【添加龙虎榜到自选】任务`)
    await taskid6_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【添加龙虎榜到自选】:已执行\n`
  }
...
//WX浏览选股题材热点
async function wxtask15() {
  console.log(`开始验证【WX浏览选股题材热点】任务状态`)
  await wxstatuid5_new()
  if (wxstatuid5_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX浏览选股题材热点】任务`)
    await wxtaskid5_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX浏览选股题材热点】:已执行\n`
  }
...
async function task16() {
  console.log(`开始验证【浏览选股题材热点】任务状态`)
  await statuid7_new()
  if (statuid7_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【浏览选股题材热点】任务`)
    await taskid7_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【浏览选股题材热点】:已执行\n`
  }
...
//WX浏览股票扫雷
async function wxtask16() {
  console.log(`开始验证【WX浏览股票扫雷】任务状态`)
  await wxstatuid6_new()
  if (wxstatuid6_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX浏览股票扫雷】任务`)
    await wxtaskid6_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX浏览股票扫雷】:已执行\n`
  }
...
async function task17() {
  console.log(`开始验证【浏览脱水研报】任务状态`)
  await statuid8_new()
  if (statuid8_new.done == 0) {
    console.log(`开始申请票据...`)
    await taskticket_new(); //申请票据
    console.log(`执行【浏览脱水研报】任务`)
    await taskid8_new(ticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【浏览脱水研报】:已执行\n`
  }
...
//WX分享个股行情
async function wxtask17() {
  console.log(`开始验证【WX分享个股行情】任务状态`)
  await wxstatuid21_new()
  if (wxstatuid21_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX分享个股行情】任务`)
    await wxtaskid21_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX分享个股行情】:已执行\n`
  }
...
async function wxtask18() {
  console.log(`开始验证【WX分享一篇资讯】任务状态`)
  await wxstatuid29_new()
  if (wxstatuid29_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX分享一篇资讯】任务`)
    await wxtaskid29_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX分享一篇资讯】:已执行\n`
  }
...
async function wxtask19() {
  ...
  if (wxstatuid2_new.done == 0) {
    console.log(`开始申请票据...`)
    await wxtaskticket_new(); //申请票据
    console.log(`执行【WX浏览行情港股】任务`)
    await wxtaskid2_new(wxticket);
  } else {
    console.log(`准备执行下一个任务...`)
    tz += `【WX浏览行情港股】:已执行\n`
  }
...
//查看今日大盘行情
function taskid1_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=11&id=1&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【查看今日大盘行情】:获得 ${task.reward_desc}\n`);
      tz += `【查看今日大盘行情】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid1_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=1&tid=11&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid1_new = JSON.parse(data)
      if (statuid1_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX查看今日大盘行情
function wxtaskid1_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=11&id=1&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX查看今日大盘行情】:获得${data.reward_desc}`);
              tz += `【WX查看今日大盘行情】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX查看今日大盘行情】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid1_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1100&id=1&tid=11&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid1_new = JSON.parse(data)
      if (wxstatuid1_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//浏览行情港股
function taskid2_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=56&id=2&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【浏览行情港股】:获得 ${task.reward_desc}\n`);
      tz += `【浏览行情港股】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid2_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=2&tid=56&_appName=ios${taskheaderVal}`,
...
//WX浏览行情港股
function wxtaskid2_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=56&id=2&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
...
function wxstatuid2_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1100&id=2&tid=56&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid2_new = JSON.parse(data)
      if (wxstatuid2_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//阅读资讯
function taskid3_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=5&id=3&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
...
function statuid3_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=3&tid=5&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid3_new = JSON.parse(data)
      if (statuid3_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX阅读资讯
function wxtaskid3_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=5&id=3&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
...
function wxstatuid3_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=3&tid=5&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid3_new = JSON.parse(data)
      if (wxstatuid3_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//查看热搜并添加自选
function taskid4_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=49&id=4&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【查看热搜并添加自选】:获得 ${task.reward_desc}\n`);
      tz += `【查看热搜并添加自选】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid4_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=4&tid=49&_appName=ios${taskheaderVal}`,
...
//WX浏览行情港股
function wxtaskid4_get(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=56&id=4&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
      //url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?id=4&tid=56&task_ticket=${ticket}&actid=1111&channel=1&action=taskdone&rndtime=${rndtime}&_appName=ios${taskheaderVal}`,
      headers: {
        'Cookie': `${taskkeyVal}`,
        'Accept': `*/*`,
        'Connection': `keep-alive`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【WX浏览行情港股】:获得 ${task.reward_desc}\n`);
      tz += `【WX浏览行情港股】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function wxstatuid4_get() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=4&tid=56&_appName=ios${taskheaderVal}`,
      //url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?id=4&tid=56&actid=1111&channel=1&action=taskstatus&_rndtime=${rndtime}&_appName=ios${taskheaderVal}`,
      headers: {
        'Cookie': `${taskkeyVal}`,
        'Accept': `*/*`,
        'Connection': `keep-alive`,
...
//WX添加自选股
function wxtaskid4_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=2&id=4&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
...
function wxstatuid4_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=4&tid=2&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid4_new = JSON.parse(data)
      if (wxstatuid4_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//添加精选到自选
function taskid5_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=60&id=5&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【添加精选到自选】:获得 ${task.reward_desc}\n`);
      tz += `【添加精选到自选】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid5_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=5&tid=60&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid5_new = JSON.parse(data)
      if (statuid5_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX浏览选股题材热点
function wxtaskid5_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=58&id=5&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX浏览选股题材热点】:获得${data.reward_desc}`);
              tz += `【WX浏览选股题材热点】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX浏览选股题材热点】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid5_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=5&tid=58&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid5_new = JSON.parse(data)
      if (wxstatuid5_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//添加龙虎榜到自选
function taskid6_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=61&id=6&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【添加龙虎榜到自选】:获得 ${task.reward_desc}\n`);
      tz += `【添加龙虎榜到自选】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid6_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=6&tid=61&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid6_new = JSON.parse(data)
      if (statuid6_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX浏览股票扫雷
function wxtaskid6_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=59&id=6&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX浏览股票扫雷】:获得${data.reward_desc}`);
              tz += `【WX浏览股票扫雷】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX浏览股票扫雷】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid6_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=6&tid=59&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid6_new = JSON.parse(data)
      if (wxstatuid6_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//浏览选股题材热点
function taskid7_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=58&id=7&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【浏览选股题材热点】:获得 ${task.reward_desc}\n`);
      tz += `【浏览选股题材热点】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid7_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=7&tid=58&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid7_new = JSON.parse(data)
      if (statuid7_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX浏览社区
function wxtaskid7_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=31&id=7&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX浏览社区】:获得${data.reward_desc}`);
              tz += `【WX浏览社区】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX浏览社区】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid7_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=7&tid=31&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid7_new = JSON.parse(data)
      if (wxstatuid7_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//浏览脱水研报
function taskid8_new(ticket) {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=48&id=8&task_ticket=${ticket}&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      let task = JSON.parse(data)
      $.log(`【浏览脱水研报】:获得 ${task.reward_desc}\n`);
      tz += `【浏览脱水研报】:获得 ${task.reward_desc}\n`
      await $.wait(10000); //等待10秒
      resolve()
    })
  })
...
function statuid8_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskstatus&channel=1&actid=1111&id=8&tid=48&_appName=ios${taskheaderVal}`,
    ...
    $.get(testurl, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      statuid8_new = JSON.parse(data)
      if (statuid8_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
//WX分享个股行情
function wxtaskid21_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=51&id=21&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX分享个股行情】:获得${data.reward_desc}`);
              tz += `【WX分享个股行情】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX分享个股行情】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid21_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=21&tid=51&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
    ...
    $.post(url, async (error, resp, data) => {
      if (logs == 1) $.log(data)
      wxstatuid21_new = JSON.parse(data)
      if (wxstatuid21_new.done == 1) {
        $.log(`验证状态失败,任务已执行🚫`);
      } else {
        $.log(`验证状态成功,可执行任务🎉`);
      }
...
function wxtaskid29_new(wxticket) {
  return new Promise((resolve) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&tid=50&id=29&task_ticket=${wxticket}&action=taskdone`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            data = JSON.parse(data);
            if(data.retcode == 0){
              $.log(`【WX分享一篇资讯】:获得${data.reward_desc}`);
              tz += `【WX分享一篇资讯】:获得${data.reward_desc}\n`
              await $.wait(5000); //等待5秒
            }else{
              console.log(`任务完成失败，错误信息：${JSON.stringify(data)}\n`)
              tz += `【WX分享一篇资讯】:${data.retmsg}\n`
            }
          }
        }
      } catch (e) {
...
function wxstatuid29_new() {
  return new Promise((resolve, reject) => {
    let url = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      body: `_h5ver=2.0.1&actid=1110&id=29&tid=50&action=taskstatus`,
      headers: {
        'Accept': `application/json, text/plain, */*`,
        'Origin': `https://wzq.tenpay.com`,
        'Accept-Encoding': `gzip, deflate, br`,
...
function taskticket_new() {
  return new Promise((resolve, reject) => {
    let testurl = {
      url: `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskticket&channel=1&actid=1111&_rndtime=${rndtime}&_appName=ios${taskheaderVal}`,
