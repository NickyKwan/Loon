...
const $ = new Env('抖一抖dou1dou'); 
 let status; 
 status = (status = ($.getval("rwstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符 
 const rwurlArr = [], rwhdArr = [],rwcount = '' 
 let times = Math.round(Date.now() / 1000) 

 let rwurl = $.getdata('rwurl') 
 let rwhd = $.getdata('rwhd') 
...
!(async () => { 
 if (typeof $request !== "undefined") { 
  ...
  await rwck()
   ...
   {
rwurlArr.push($.getdata('rwurl')) 
 rwhdArr.push($.getdata('rwhd')) 

 let rwcount = ($.getval('rwcount') || '1'); 
 for (let i = 2; i <= rwcount; i++) { 
 rwurlArr.push($.getdata(`rwurl${i}`)) 
 rwhdArr.push($.getdata(`rwhd${i}`)) 
 } 
 
 console.log(`------------- 共${rwhdArr.length}个账号-------------\n`) 
 for (let i = 0; i < rwhdArr.length; i++) { 
 if (rwhdArr[i]) { 
   rwurl = rwurlArr[i]; 
 rwhd = rwhdArr[i]; 
 $.index = i + 1; 
...
//await $.wait(1000);
 ...
 //数据获取 https://activity3-aweme-lq.awemeughun.com/aweme/ughun/activity/mayday/hongbao_lottery/?version_code=15.7.0&js_sdk_version=2.6.0.7&tma_jssdk_version=2.6.0.7&app_name=aweme&app_version=15.7.0&vid=A99C05B1-F594-442E-B146-E31853BD39F8&device_id=36662104633&channel=App%20Store&slide_guide_has_shown=1&mcc_mnc=46001&aid=1128&minor_status=0&screen_width=1242&openudid=78ec57565bb66207f3ecb526886c15d05069dfb9&cdid=69ED3966-EFC0-4CA4-AEFB-F849905B32AB&os_api=18&ac=WIFI&os_version=14.3&appTheme=dark&device_platform=iphone&build_number=157011&is_vcd=1&device_type=iPhone9,2&iid=580992789137280&idfa=41E57B04-6883-4262-8929-439B4E505781
      ...
      function rwck() { 
 if ($request.url.indexOf("ughun/activity/mayday/dou/lottery") > -1) { 
 const rwurl = $request.url 

 if(rwurl) $.setdata(rwurl,`rwurl${status}`) 
 $.log("rwurk:"+rwurl) 
 const rwhd = JSON.stringify($request.headers) 
 if(rwhd) $.setdata(rwhd,`rwhd${status}`) 
 $.log(rwhd) 
 $.msg($.name,"",'dou1dou抖一抖'+`${status}` +'数据获取成功！') 
 } 
 }
   ...
   $.post(plant6_url,async(error, response, data) =>{
    try{
        ...
        //if(result.status_code  == 0)
          ...
          //if(result.dou_lottery_info.button_info.text == "邀请1位朋友")
         // token = result.button_info.text
         // $.msg(result.data.task_token)
          //$.msg("邀请1位朋友"+token)
        if(result.status_code != 0)
   ...
   $.post(plant6_url,async(error, response, data) =>{
    try{
        ...
        if(result.status_code  == 0)
          ...
          token1 = data.match(/task_token":"(.*?)"/)[1]
...
async function rwtk1(){
 return new Promise((resolve) => {
     url1 = rwurl.replace('dou/lottery','hongbao_lottery')
    let plant6_url = {
   		url: url1,
        headers: JSON.parse(rwhd),
