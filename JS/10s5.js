...
10s阅读
微信打开
立即参与 -> http://h5.jrkuaixun.xyz/j/r2?upuid=137149&ch=gzh
备用链接 -> http://h5.njchenyue.xyz/j/r2?upuid=137149&ch=gzh
...
let cookiesArr = [$.getdata('read10sck5')]
if ($.isNode()) {
    ...
    !(async () => {
        ...
        if (!cookiesArr[0]) {
            $.msg($.name, '【提示】请先获取cookie', 'http://h5.jrkuaixun.xyz/j/r2?upuid=137149&ch=gzh', {
                "open-url": "http://h5.jrkuaixun.xyz/j/r2?upuid=137149&ch=gzh"
            });
            return;
        }
...
//        $.msg($.name, "", '10s阅读 获取数据获取成功！'+read10surl)
        ...
        if ($request.headers.Cookie) $.setdata($request.headers.Cookie, `read10sck5`)
        $.log(read10sck5)
